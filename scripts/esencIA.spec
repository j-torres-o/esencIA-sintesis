...
# PyInstaller provides SPECPATH pointing to the directory containing this spec file
SPEC_DIR = Path(SPECPATH) if 'SPECPATH' in globals() else Path(os.getcwd()) / 'scripts'
ROOT_DIR = SPEC_DIR.parent.resolve()
...
datas += [(str(ROOT_DIR / 'src' / 'ui'), 'src/ui')]
...
exe = EXE(
    ...
    icon=[str(ROOT_DIR / 'src' / 'ui' / 'assets' / 'icon.ico')],
)
